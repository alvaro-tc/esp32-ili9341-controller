...
#include <lvgl.h>
#include <ui.h>  
#include "RCCarController.h"  // Agregar controlador de auto RC
#define TFT_LED 38
...
void setup() {
  ...
  Serial.begin(115200);  // Aumentar velocidad para mejor debugging
  while (!Serial) delay(10);

  Serial.println("=== Controlador Auto RC Beta01 ===");
  ...
  lv_disp_draw_buf_init( &draw_buf, buf, NULL, screenWidth * screenHeight / 10 );
    ...
    // Inicializar controlador de auto RC
    if (carController.begin()) {
        Serial.println("✅ Sistema Auto RC listo");
        showWelcomeScreen();
    } else {
        Serial.println("❌ Error inicializando Auto RC");
        showErrorScreen();
    }
}
...
void loop(void) {
    ...
    // Actualizar controlador de auto RC (cada 30ms)
    if (millis() - lastControlUpdate >= 30) {
        carController.update();
        lastControlUpdate = millis();
    }
    
    // Actualizar pantalla con información del auto (cada 100ms)
    ...
    if (millis() - lastDebug >= 500) {
        carController.printControlValues();
        lastDebug = millis();
    }
}
